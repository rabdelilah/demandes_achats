		...
		<div class="wrap">
            ...
            <div id="content">
                ...
                <div id="main">
                    ...
                    <!--               main             -->
                    <h3>Saisie Damande D'achat</h3>
                    <%if(request.getParameter("mode").equalsIgnoreCase("addDoc")){ %>
                    <form action="action" method="post">
                    <table>
                    	<tr>
                    		<td>Date : <input type="text" name="date_doc" class="datepicker" /></td>
                    		<td>Dépôt : 
                    		<select name="depot">
							<%
								List<String[]> depot=services.getDepot();
                    			for(int i=0;i<depot.size();i++){%>
                    			<option value="<%=depot.get(i)[0]%>"><%=depot.get(i)[1] %></option>
                    		<%	}
                    		%>
                    		</select>
							</td>
							<td>Date Livraison: <input type="text" name="date_livraison" class="datepicker" /></td>
                    	</tr>
                    	<tr>
							...
							<%
								List<String[]> fournisseur=services.getFournisseur();
                    			for(int i=0;i<fournisseur.size();i++){%>
                    			<option value="<%=fournisseur.get(i)[0]%>"><%=fournisseur.get(i)[1] %></option>
                    		<%	}
                    		%>
                    		</select>
							</td>
							<td>Acheteur : <input type="text" name="acheteur"  readonly="readonly" value="<%=session.getAttribute("login") %>" /></td>
                    	</tr>
                    	<tr>
                    		<td colspan="2">
                    			<input type="hidden" name="op" value="add_da" />
                    			<input type="submit" class="ok" value="Enregistrer">
                    		</td>
                    	</tr>
                    </table>
                    </form>
                    <%}else if(request.getParameter("mode").equalsIgnoreCase("editDoc")){ 
                    ...
                    <table width='100%' cellpadding='0' cellspacing='0' border='0' class='tablesorter hasFilters'> 
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="2">
									<input type="hidden" name="op" value="add_ligne"/>
							...
							<tr>
								...
								<td colspan="2">
									<input type="hidden" name="op" value="edit_ligne"/>
									<input type="hidden" name="id_doc" value="<%=request.getParameter("id")%>"/>
							...
							<tr>
								...
								<%} %>
								<td><a href="add_da.jsp?mode=editDoc&mode2=editLigne&id=<%=request.getParameter("id") %>&id_ligne=<%=donnee.get(i)[0] %>"><img src="img/i_edit.png"/></a></td>
								<td><a href="action?op=delete_ligne&id_doc=<%=request.getParameter("id") %>&id_ligne=<%=donnee.get(i)[0] %>"><img src="img/i_delete.png"/></a></td>
							</tr>
							<%} %>
						</tbody>
					</table>
